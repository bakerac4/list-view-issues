...
const maxHeight = 500;
export default class HomeFeedVideoItem extends Component<Args> {
    @tracked item = this.args.item;
    videoView: any;
    event: { id: number; unsubscribe: () => void };
    ...
    toggleColor(args) {
        ...
        // unmute the video when showing the original
        const item = this.args.item as any;
        if (item.isVideo && this.videoView) {
